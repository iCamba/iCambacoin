iCambaCoin-qt: Qt4 GUI for iCambaCoin
===============================
...
Alternatively, install Qt Creator and open the `iCambaCoin-qt.pro` file.

An executable named `iCambaCoin-qt` will be built.
...
A warning for people using the *static binary* version of iCambaCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of iCambaCoin is linked against libdb5.3.
...
installed causes iCambaCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
